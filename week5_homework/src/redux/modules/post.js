...
export const updatePost = (post) => {
  console.log("업데이트 포스생성완료!", post);

  return { type: UPDATE, post };
};
export const logoutId = (post) => {
  console.log("로그아웃 포스생성완료!", post);

  return { type: LOGOUT, post };
};
// middlewares
export const loadPostFB = (user) => {
  return async function (dispatch) {
    console.log("미들웨어동작중",user)
    if(user){
...
export const deletePostFB = (post) => {
  return async function (dispatch) {
    ...
    const user_docs = await getDocs(
      query(collection(db, "post"), where("date", "==", post))
    );
    let temp = "";
    user_docs.forEach((v) => {
      temp = v.id;
    ...
    await deleteDoc(doc(db, "post", temp));
...
export const updatePostFB = (post) => {
  return async function (dispatch) {
    console.log(post)
    const user_docs = await getDocs(
      query(collection(db, "post"), where("date", "==", post.date))
    );
    let temp = "";
    user_docs.forEach((v) => {
      temp = v.id;
    });
    console.log("이거출력해줘잉",temp)
    await setDoc(doc(db,"post", temp),post)

    dispatch(updatePost(post));

    console.log("미드웨어 업데이트", post);
  };
};
// 리듀서예요.
...
export default function reducer(state = initialState, action = {}) {
  switch (action.type) {
    ...
    case "post/DELETE": {
      ...
      const new_post_list = state.list.filter((v) => v.date !== action.post);
      console.log("이거확인할 차례임", state, new_post_list);
      return { ...state, list: new_post_list };
    }
    case "post/UPDATE":{
      console.log("여긴 리듀서",state,action.post)
      const new_post_list = state.list.filter((v) => v.date !== action.post.date);
      new_post_list.push(action.post)
      console.log("이거확인할 차례임", state, new_post_list);
      return { ...state, list: new_post_list };
    }
    case "user/LOGOUT":{
      console.log("리듀서 로그아웃 왔음?",initialState)
      return initialState;
    }
